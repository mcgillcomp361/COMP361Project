...
class Player(DirectObject.DirectObject):
    ...
    def __init__(self, name):
        ...
        self.accept('u', self._testAddUnit)
        self.accept('s', self._testAddStructure)
    
    def _testAddUnit(self):
        if self.selected_planet:
            self.selected_planet.task_unit_timer = taskMgr.doMethodLater(0.01, self._delayedConstructUnit, 'buildSwarm', extraArgs =[Swarm, self.selected_planet], appendTask=True)
    
    def _testAddStructure(self):
        if self.selected_planet:
            self.selected_planet.task_structure_timer = taskMgr.doMethodLater(1.0, self._delayedConstructStructure, 'buildForge', extraArgs =[Forge, self.selected_planet], appendTask=True)
